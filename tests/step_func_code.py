...
def izh_soma_step_func_double_buffer(tick, agent_index, globals, agent_ids, breeds, locations, connectivity, neuron_params, learning_params, internal_state, internal_learning_state, synapse_history, input_spikes_tensor, output_spikes_tensor, internal_states_buffer, internal_learning_states_buffer, write_internal_state, write_output_spikes_tensor, write_internal_states_buffer):
    ...
    write_internal_states_buffer[agent_index][t_current][0] = v
    write_internal_states_buffer[agent_index][t_current][1] = u

@jit.rawkernel(device='cuda')
def lif_soma_step_func_double_buffer(tick, agent_index, globals, agent_ids, breeds, locations, connectivity, neuron_params, learning_params, internal_state, internal_learning_state, synapse_history, input_spikes_tensor, output_spikes_tensor, internal_states_buffer, internal_learning_states_buffer, write_internal_state, write_output_spikes_tensor, write_internal_states_buffer):
    synapse_indices = locations[agent_index]
    I_synapse = 0.0
    for i in range(len(synapse_indices)):
        synapse_index = int(synapse_indices[i])
    ...
    write_output_spikes_tensor[agent_index][t_current] = s
    write_internal_states_buffer[agent_index][t_current][0] = v
    write_internal_states_buffer[agent_index][t_current][1] = internal_state[agent_index][1] + 1
    write_internal_states_buffer[agent_index][t_current][2] = tlast

@jit.rawkernel(device='cuda')
def synapse_single_exp_step_func_double_buffer(tick, agent_index, globals, agent_ids, breeds, locations, connectivity, synapse_params, learning_params, internal_state, internal_learning_state, synapse_history, input_spikes_tensor, output_spikes_tensor, internal_states_buffer, internal_learning_states_buffer, write_internal_state, write_output_spikes_tensor, write_internal_states_buffer):
    t_current = int(tick)
    dt = globals[0]
    ...
    pre_soma_id, post_soma_id = (connectivity[agent_index][0], connectivity[agent_index][1])
    spike = get_soma_spike(tick, agent_index, globals, agent_ids, pre_soma_id, t_current, input_spikes_tensor, output_spikes_tensor)
    ...
    write_internal_states_buffer[agent_index][t_current][0] = I_synapse
    write_internal_states_buffer[agent_index][t_current][1] = spike
    write_internal_states_buffer[agent_index][t_current][2] = t_current

@jit.rawkernel(device='cuda')
def learning_rule_selector_double_buffer(tick, agent_index, globals, agent_ids, breeds, locations, connectivity, synapse_params, learning_params, internal_state, internal_learning_state, synapse_history, input_spikes_tensor, output_spikes_tensor, internal_states_buffer, internal_learning_states_buffer, write_internal_state, write_output_spikes_tensor, write_internal_states_buffer):
    stdpType = learning_params[agent_index][0]
    if stdpType == -1:
        pass
    elif stdpType == 0:
        exp_pair_wise_stdp(tick, agent_index, globals, agent_ids, breeds, locations, connectivity, synapse_params, learning_params, internal_state, internal_learning_state, synapse_history, input_spikes_tensor, output_spikes_tensor, internal_states_buffer, internal_learning_states_buffer)

@jit.rawkernel(device='cuda')
def stepfunc(
global_tick,
device_global_data_vector,
a0,a1,a2,a3,a4,a5,a6,a7,a8,a9,a10,a11,write_a5,write_a9,write_a10,
