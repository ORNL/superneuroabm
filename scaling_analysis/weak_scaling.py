...
def inject_poisson_drive(model, input_synapses, rate_hz, ticks, dt_ms, seed):
    ...
    ``input_synapses`` is owned by this rank (the list is built from this rank's own
    synapse ids), so no MPI is needed, and each synapse's whole spike train is written
    in a single round-trip. The collective add_spike/add_spike_list would deadlock here
    because each rank injects a DIFFERENT set of local synapses (and a different number
    of spikes) — collective calls require every rank to call in lockstep with the same id.
    """
...
def main():
    ...
    if args.partition_dir:
        ...
        # Shared filesystem (not /tmp, which is node-local on Frontier)
    ...
    if args.csv and rank == 0:
        ...
        with open(csv_path, 'a', newline='') as f:
            ...
            if write_header:
                writer.writerow([
                    'job_id', 'nodes', 'workers', 'neurons', 'synapses', 'in_degree',
                    'ticks', 'generation_time', 'model_creation_time', 'gpu_setup_time',
                    'construction_time', 'simulation_time', 'total_time'
                ])
            writer.writerow([
                os.environ.get('SLURM_JOB_ID', ''),
                os.environ.get('SLURM_NNODES', ''),
                size, total_neurons, total_synapses, K, simulation_ticks,
